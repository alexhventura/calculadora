    ...
    <div className="min-h-screen flex flex-col bg-slate-50 font-sans antialiased text-slate-950 overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto w-full flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <Link to={ROUTES.home} className="flex items-center gap-2.5" aria-label={`${SITE_DOMAIN} — Início`}>
            ...
            <div>
              ...
              <span className="text-[10px] font-semibold text-slate-500 block -mt-1 tracking-wider uppercase">{SITE_TAGLINE}</span>
            </div>
          </Link>
          <nav className="flex flex-wrap items-center justify-center gap-4 text-xs font-semibold text-slate-600" aria-label="Navegação principal">
            ...
            <Link to={ROUTES.conversorMoedas} className="hover:text-[#800020] transition-colors">Conversor de Moedas</Link>
      ...
      <footer className="bg-slate-900 text-slate-300 py-10 px-6 md:px-12 border-t border-slate-800 text-xs text-center mt-auto" role="contentinfo">
        <div className="max-w-7xl mx-auto w-full flex flex-col items-center gap-6">
          ...
          <p className="max-w-3xl text-[11px] text-slate-400 leading-relaxed font-sans">
            Conteúdo educativo. Não constitui recomendação de investimento ou assessoria profissional.
          </p>
          <div className="text-[10px] text-slate-400 font-mono">© 2026 {SITE_DOMAIN}</div>
        </div>
      </footer>
    </div>
  );
